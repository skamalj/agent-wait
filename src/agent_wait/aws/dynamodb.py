...
class DynamoDbAnnounce(BaseAnnounce):
    ...
    def __init__(
        ...
        *,
        ...
        # The stubs are a dev convenience; a caller may hand in any Table-shaped object.
        self._table: Any = table or boto3.resource(  # pyright: ignore[reportUnknownMemberType]
    ...
    def deliver(self, envelope: WaitEnvelope, transition: Transition) -> None:
        ...
        for key in ("default", "answer_ttl", "source", "reply_to"):
            value: Any = getattr(envelope, key)
            if value:
                item[key] = dict(cast(Mapping[str, Any], value)) if isinstance(value, Mapping) else value
